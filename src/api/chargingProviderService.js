...
const chargingProviderService = {
  ...
  // Giriş yapmış kullanıcının şarj geçmişini getir
  getUserChargingHistory: async () => {
    try {
      const response = await api.get('sarj-gecmisi/');
      return response.data;
    } catch (error) {
      console.error('Şarj geçmişi alınırken hata:', error);
      throw error;
    }
  },

  // Kullanıcının şarj geçmişini kaydet
  saveChargingHistory: async (data) => {
    try {
      const response = await api.post('sarj-gecmisi/', data);
      return response.data;
    } catch (error) {
      console.error('Şarj geçmişi kaydedilirken hata:', error);
      throw error;
    }
  }
};
